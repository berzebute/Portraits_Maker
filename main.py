...
class PortraitMaker:
    ...
    def setup_ui(self):
        ...
        tk.Label(self.ctrl_panel, text="저장 파일명", font=("Malgun Gothic", 11, "bold"), fg=self.accent_color, bg=self.bg_panel).pack(anchor="w", pady=(0, 5))
        ...
        ttk.Radiobutton(self.color_frame, text="흰색", variable=self.frame_color_var, value="#ffffff", command=self.update_rect_color, style="TRadiobutton").pack(side="left", padx=(0, 20))
        ttk.Radiobutton(self.color_frame, text="빨간색", variable=self.frame_color_var, value="#ff0000", command=self.update_rect_color, style="TRadiobutton").pack(side="left")
    ...
    def update_rect_color(self):
        if hasattr(self, 'rect_id') and self.canvas.find_withtag(self.rect_id):
            self.canvas.itemconfig(self.rect_id, outline=self.frame_color_var.get())
    ...
    def save_portraits(self):
        # 저장 시점에서 다시 한 번 정규식을 통해 검증된 이름을 가져옵니다.
        raw_name = self.char_name_var.get().strip()
        name = re.sub(r'[^a-zA-Z0-9_]', '', raw_name)
        
        if not name:
            messagebox.showwarning("경고", "유효한 캐릭터 이름을 입력해주세요.")
            return
    ...
    def limit_char_name(self, *args):
        # 입력 시점에 영문, 숫자, 언더바만 남기도록 필터링
        value = self.char_name_var.get()
        # [^a-zA-Z0-9_]는 '영문, 숫자, _ 가 아닌 것'을 의미하므로 이를 제거함
        filtered = re.sub(r'[^a-zA-Z0-9_]', '', value)[:15]
        if value != filtered:
